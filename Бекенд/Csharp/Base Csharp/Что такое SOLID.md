...
**O  (open cloded priniciple - принцип открытости закрытости)** - Класс должен закрыт для изменения, но открыт для расширения (н-р, через перегрузки, которые еще называют ad-hoc полиморфизмом).
![[Solid-O.png]]
